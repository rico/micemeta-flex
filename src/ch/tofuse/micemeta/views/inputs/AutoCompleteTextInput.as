package ch.tofuse.micemeta.views.inputs
{
	...
	import ch.tofuse.micemeta.skins.input.AutoCompleteTextInputSkin;
	...
	public class AutoCompleteTextInput extends SkinnableComponent
	{
		...
		public function AutoCompleteTextInput()
		{
			...
			this.setStyle("skinClass", Class(AutoCompleteTextInputSkin));
		...
		[Bindable] public var widthInChars:Number = NaN;
		...
		override protected function partAdded(partName:String, instance:Object) : void{
			...
			if (instance==inputTxt){
				...
				if( !isNaN(widthInChars) ) {
					inputTxt.widthInChars = widthInChars;
				}
			}
			if (instance==list){
		...
		public function filterData():void
		{
			...
			collection.filterFunction = labelsFilterFunction;
		...
		private function onKeyDown(event: KeyboardEvent) : void{
			...
			if (popUp.displayPopUp){
				switch (event.keyCode){
					...
					case Keyboard.ENTER:
						acceptCompletion();
						dispatchEvent( new AutoCompleteTextInputEvent( AutoCompleteTextInputEvent.CONFIRM_SELECTION, selectedItem) )
						break;
					case Keyboard.TAB:
						if (requireSelection)
							acceptCompletion();
		...
		public function acceptCompletion() : void
		{
			if (list.selectedIndex >= 0 && collection.length>0)
			{
				...
				selectedItem = collection.getItemAt(_selectedIndex)
				
				text = returnFunction( selectedItem )
				
				inputTxt.selectRange(inputTxt.text.length, inputTxt.text.length)
				
				var e:AutoCompleteTextInputEvent = new AutoCompleteTextInputEvent( AutoCompleteTextInputEvent.SELECT, selectedItem)
				dispatchEvent(e)
